...
async def on_wishes(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            "Я могу создать сразу несколько видео в этом же стиле с твоим товаром! 🎬\n"
            "Они будут с разными героями, локациями и подачей.\n\n"
            "<b>Выберите количество:</b>\n"
            "⏱ <i>Чем больше видео — тем больше времени на генерацию, но всё делается параллельно</i>",
            reply_markup=kb_video_count(),
            parse_mode=PARSE_MODE,
        )
    except Exception as e:
...
async def confirm_generation(cb: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if success_count > 0:
            ...
            # Избегаем дублирующих уведомлений. Сообщение о старте отправляет worker при первой попытке.
            await cb.message.answer(
                f"✅ <b>Принял!</b>\n\n"
                f"🔄 <b>Прохожу модерацию Sora 2...</b>\n\n"
                f"⏱ После одобрения — сразу пришлю результаты сюда.\n"
                f"Обычно это занимает <b>5-10 минут</b> ⏳",
                parse_mode=PARSE_MODE,
            )
        else:
