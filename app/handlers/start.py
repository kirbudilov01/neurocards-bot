...
async def start_handler(message: Message):
    ...
    try:
        if WELCOME_VIDEO_FILE_ID:
            ...
            logger.info(f"📹 Sending video using file_id: {WELCOME_VIDEO_FILE_ID[:20]}...")
            await message.answer_video(video=WELCOME_VIDEO_FILE_ID)
            logger.info("✅ Video sent successfully via file_id")
        else:
            # Загружаем с диска (медленно! 60+ секунд)
            logger.info(f"📹 Sending video from {WELCOME_VIDEO_PATH} (first time upload - will be slow!)")
            ...
            logger.warning(f"⚠️ SAVE THIS file_id to WELCOME_VIDEO_FILE_ID env variable: {msg.video.file_id}")
