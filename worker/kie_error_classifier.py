...
def classify_kie_error(info: Dict[str, Any]) -> tuple[KieErrorType, str]:
    ...
    # Явно считаем 5xx как временную ошибку (retryable)
    if fail_code is not None and fail_code >= 500:
        return (KieErrorType.TEMPORARY, f"fail_code={fail_code}")
    if status_code is not None and status_code >= 500:
        return (KieErrorType.TEMPORARY, f"HTTP {status_code}")
...
def get_user_error_message(error_type: KieErrorType) -> str:
    ...
    if error_type == KieErrorType.USER_VIOLATION:
        return (
            ...
            "🚫 На фото есть <b>люди</b> — нейросеть не работает с реалистичными людьми\n"
            "🚫 Запрещённый, взрослый или провокационный контент\n"
            "🚫 Оружие, насилие или другие нарушения политики\n\n"
            "💡 <b>Что делать:</b>\n"
            "✓ Загрузите другое фото (без людей)\n"
            "✓ Измените описание товара\n"
            "✓ Попробуйте более простой промпт\n\n"
            "💰 <b>1 кредит вернули на баланс</b> ✅"
        )
    ...
    if error_type == KieErrorType.TEMPORARY:
        return (
            "❌ <b>Sora 2 перегружена</b>\n\n"
            "Мы попробовали 3 раза, но сервис не отвечает.\n"
